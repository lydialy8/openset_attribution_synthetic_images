...
def paths_and_labels_to_dataset(
    ...
    balanced_path_dataset = tf.data.experimental.choose_from_datasets(
        paths_datasets, choice_dataset
    )
    if safe_triplet:
    ...
    balanced_img_dataset = balanced_path_dataset.map(lambda x, y: load_image(x, y, *args))
...
def load_image(
        path, label, image_size, num_channels, interpolation, augmentation_enabled, crop_to_aspect_ratio=False
):
...
def augmentation(img):
    img = tf.cond(tf.math.greater(0.3, tf.random.uniform(shape=(1, 1), minval=0, maxval=1, dtype=tf.float32, seed=None,
                                                         name=None)),
                  lambda: tf.image.random_jpeg_quality(img, 70, 100), lambda: img)
    img = tf.cond(tf.math.greater(0.3, tf.random.uniform(shape=(1, 1), minval=0, maxval=1, dtype=tf.float32, seed=None,
                                                         name=None)), lambda: tf.image.random_saturation(img, 0.5, 1.0),
                  lambda: img)
    img = tf.cond(tf.math.greater(0.3, tf.random.uniform(shape=(1, 1), minval=0, maxval=1, dtype=tf.float32, seed=None,
                                                         name=None)), lambda: tf.image.random_hue(img, 0.2),
                  lambda: img)
    img = tf.cond(tf.math.greater(0.3, tf.random.uniform(shape=(1, 1), minval=0, maxval=1, dtype=tf.float32, seed=None,
                                                         name=None)), lambda: tf.image.random_brightness(img, 0.2),
                  lambda: img)
    img = tf.cond(tf.math.greater(0.3, tf.random.uniform(shape=(1, 1), minval=0, maxval=1, dtype=tf.float32, seed=None,
                                                         name=None)), lambda: tf.image.random_contrast(img, 0.2, 0.5),
                  lambda: img)
    img = tf.cond(tf.math.greater(0.3, tf.random.uniform(shape=(1, 1), minval=0, maxval=1, dtype=tf.float32, seed=None,
                                                         name=None)), lambda: tf.image.random_flip_left_right(img),
                  lambda: img)
    img = tf.cond(tf.math.greater(0.3, tf.random.uniform(shape=(1, 1), minval=0, maxval=1, dtype=tf.float32, seed=None,
                                                         name=None)), lambda: tf.image.random_flip_up_down(img),
                  lambda: img)
    return img
