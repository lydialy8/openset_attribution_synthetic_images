...
import numpy as np
from cswin_transformer.residual_block import make_basic_block_layer, make_bottleneck_layer
...
class conv_patching(tf.keras.layers.Layer):
    def __init__(self, filter_num, stride=1, **kwargs):
        super(conv_patching, self).__init__(**kwargs)
        self.filter_num = filter_num
        self.stride = stride
        self.conv1 = tf.keras.layers.Conv2D(filters=filter_num // 2,
                                            kernel_size=(3, 3),
                                            strides=1,
                                            padding="same")
        ...
        self.layer1 = make_basic_block_layer(filter_num=filter_num // 2,
                                             blocks=3)
        self.layer2 = make_basic_block_layer(filter_num=filter_num,
                                             blocks=4,
                                             stride=2)
    ...
    def get_config(self):
        config = super(conv_patching, self).get_config()
...
class Mlp(tf.keras.layers.Layer):
    def __init__(self, n_feature, n_hidden_feature=None, activation=tfa.activations.gelu, dropout_rate=0., **kwargs):
    ...
    def build(self, input_shape):
        self.fc1 = tf.keras.layers.Dense(
            self.n_hidden_feature if self.n_hidden_feature is not None else input_shape[-1], name="fc1")
...
class WindowAttention(tf.keras.layers.Layer):
    def __init__(self, n_head, window_size, scale=None, use_bias=True, dropout_rate=0., attention_dropout_rate=0.,
                 **kwargs):
    ...
    def build(self, input_shape):
        ...
        self.relative_position_bias_table = self.add_weight("relative_position_bias_table", shape=[
            ((2 * self.window_size[0]) - 1) * ((2 * self.window_size[1]) - 1), self.n_head], dtype=self.dtype,
        ...
        self.relative_position_index = tf.Variable(tf.convert_to_tensor(relative_position_index), trainable=False,
    ...
    def call(self, inputs, mask=None):
        ...
        if mask is not None:
            ...
            attn = tf.reshape(attn, [-1, n_window, self.n_head, n, n]) + tf.cast(
...
class WindowAttentionV2(tf.keras.layers.Layer):
    def __init__(self, n_head, window_size, use_bias=True, dropout_rate=0., attention_dropout_rate=0.,
                 pre_window_size=0, **kwargs):
        ...
        self.dropout_rate = dropout_rate
        self.attention_dropout_rate = attention_dropout_rate
        self.pre_window_size = pre_window_size if not isinstance(pre_window_size, int) else [pre_window_size,
                                                                                             pre_window_size]
    ...
    def build(self, input_shape):
        self.logit_scale = self.add_weight("logit_scale", shape=[self.n_head, 1, 1],
                                           initializer=tf.keras.initializers.Constant(np.log(10.)), dtype=self.dtype,
                                           trainable=self.trainable)
        self.cpb_mlp1 = tf.keras.layers.Dense(512, use_bias=True, activation=tf.keras.activations.relu,
                                              name="cpb_mlp_0")
        ...
        relative_coords_table = np.expand_dims(
            np.transpose(np.stack(np.meshgrid(relative_coords_h, relative_coords_w, indexing="ij")), [1, 2, 0]),
            axis=0)  # 1, 2*Wh-1, 2*Ww-1, 2
        ...
        relative_coords_table = np.sign(relative_coords_table) * np.log2(np.abs(relative_coords_table) + 1.0) / np.log2(
            8)
        self.relative_coords_table = self.add_weight("relative_coords_table", shape=np.shape(relative_coords_table),
        ...
        self.project = tf.keras.layers.Dense(input_shape[-1], use_bias=True, name="proj")
        if self.use_bias:
            self.q_bias = self.add_weight("q_bias", shape=[input_shape[-1]], initializer=tf.keras.initializers.Zeros,
                                          dtype=self.dtype, trainable=self.trainable)
            self.v_bias = self.add_weight("v_bias", shape=[input_shape[-1]], initializer=tf.keras.initializers.Zeros,
                                          dtype=self.dtype, trainable=self.trainable)
    ...
    def call(self, inputs, mask=None):
        ...
        relative_position_bias_table = tf.reshape(self.cpb_mlp2(self.cpb_mlp1(self.relative_coords_table)),
                                                  [-1, self.n_head])
...
class SwinTransformerBlock(tf.keras.layers.Layer):
    def __init__(self, image_shape, n_head, window_size, shift_size=0, ratio=4., scale=None, use_bias=True,
                 dropout_rate=0., attention_dropout_rate=0., droppath_rate=0., normalize=normalize,
                 activation=tfa.activations.gelu, **kwargs):
        ...
        self.attn = WindowAttention(n_head, self.window_size, scale, use_bias, dropout_rate, attention_dropout_rate,
                                    name="attn")
        if 0 < droppath_rate:
    ...
    def build(self, input_shape):
        self.mlp = Mlp(input_shape[-1], int(input_shape[-1] * self.ratio), self.activation, self.dropout_rate,
                       name="mlp")
        self.attn_mask = None
...
class SwinTransformerBlockV2(tf.keras.layers.Layer):
    def __init__(self, image_shape, n_head, window_size, shift_size=0, ratio=4., use_bias=True, dropout_rate=0.,
                 attention_dropout_rate=0., droppath_rate=0., normalize=normalize, activation=tfa.activations.gelu,
                 pre_window_size=0, **kwargs):
        ...
        self.norm1 = normalize(name="norm1")
        self.attn = WindowAttentionV2(n_head, self.window_size, use_bias, dropout_rate, attention_dropout_rate,
                                      pre_window_size, name="attn")
        if 0 < droppath_rate:
            self.droppath = DropPath(droppath_rate, name="drop_path")
        self.norm2 = normalize(name="norm2")
...
class BasicLayer(tf.keras.layers.Layer):
    def __init__(self, image_shape, n_block, n_head, window_size, ratio=4., scale=None, use_bias=True, dropout_rate=0.,
                 attention_dropout_rate=0., droppath_rate=0., normalize=normalize, activation=tfa.activations.gelu,
        ...
        self.blocks = [SwinTransformerBlock(image_shape, n_head, window_size,
                                            ...
                                            droppath_rate=droppath_rate[i] if not (
                                                        isinstance(droppath_rate, float) or isinstance(droppath_rate,
                                                                                                       int)) else droppath_rate,
...
class BasicLayerV2(tf.keras.layers.Layer):
    def __init__(self, image_shape, n_block, n_head, window_size, ratio=4., use_bias=True, dropout_rate=0.,
                 attention_dropout_rate=0., droppath_rate=0., normalize=normalize, activation=tfa.activations.gelu,
                 downsample=None, pre_window_size=0, **kwargs):
        ...
        self.blocks = [SwinTransformerBlockV2(image_shape, n_head, window_size,
                                              ...
                                              pre_window_size=pre_window_size, name="blocks_{0}".format(i))
                       for i in range(n_block)]
...
class PatchEmbed(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        out = tf.reshape(out, [-1, out.shape[1] * out.shape[2], self.n_feature])
...
def swin_transformer(x, n_class=1000, include_top=True, patch_size=4, n_feature=96, n_blocks=[2, 2, 6, 2],
                     n_heads=[3, 6, 12, 24], window_size=7, ratio=4., scale=None, use_bias=True, patch_normalize=True,
                     dropout_rate=0., attention_dropout_rate=0., droppath_rate=0.1, normalize=normalize,
                     activation=tfa.activations.gelu, conv=True):
    ...
    if conv:
        out = conv_patching(n_feature)(x)  ### input_feat: (H/4)x(W/4)x48
        out = tf.reshape(out, [-1, out.shape[1] * out.shape[2], n_feature])
    else:
    ...
    for i in range(len(n_blocks)):
        out = BasicLayer([patch_shape[0] // (2 ** i), patch_shape[1] // (2 ** i)], n_blocks[i], n_heads[i], window_size,
                         ratio, scale, use_bias, dropout_rate, attention_dropout_rate,
                         droppath_rate[sum(n_blocks[:i]):sum(n_blocks[:i + 1])], normalize, activation,
                         downsample=PatchMerging if i < (len(n_blocks) - 1) else None, name="layers_{0}".format(i))(out)
...
def swin_transformer_v2(x, n_class=1000, include_top=True, patch_size=4, n_feature=96, n_blocks=[2, 2, 6, 2],
                        n_heads=[3, 6, 12, 24], window_size=7, ratio=4., use_bias=True, patch_normalize=True,
                        dropout_rate=0., attention_dropout_rate=0., droppath_rate=0.1, normalize=normalize,
                        activation=tfa.activations.gelu, pre_window_size=0, conv=True):
    patch_size = patch_size if not isinstance(patch_size, int) else [patch_size, patch_size]
    h, w = tf.keras.backend.int_shape(x)[-3:-1]
    patch_shape = [h // patch_size[0], w // patch_size[1]]
    ...
    if 0 < dropout_rate:
        out = tf.keras.layers.Dropout(dropout_rate, name="pos_drop")(out)
    for i in range(len(n_blocks)):
        out = BasicLayerV2([patch_shape[0] // (2 ** i), patch_shape[1] // (2 ** i)], n_blocks[i], n_heads[i],
                           window_size, ratio, use_bias, dropout_rate, attention_dropout_rate,
                           droppath_rate[sum(n_blocks[:i]):sum(n_blocks[:i + 1])], normalize, activation,
                           downsample=PatchMergingV2 if i < (len(n_blocks) - 1) else None,
                           pre_window_size=pre_window_size[i], name="layers_{0}".format(i))(out)
    out = normalize(name="norm")(out)  # b, hw, c
...
def load_weight(keras_model, torch_url):
    ...
    keys = [
        w.name.split(":")[0].replace("/", "_").replace("kernel", "weight").replace("gamma", "weight").replace("beta",
                                                                                                              "bias")
...
def swin_transformer_tiny_224_w7_1k(input_tensor=None, input_shape=None, window_size=7, classes=1000, include_top=True,
                                    dropout_rate=0., attention_dropout_rate=0., droppath_rate=0.2, normalize=normalize,
                                    activation=tfa.activations.gelu, weights="imagenet", conv=True):
    ...
    out = swin_transformer(img_input, classes, include_top, patch_size=4, n_feature=96, n_blocks=[2, 2, 6, 2],
                           n_heads=[3, 6, 12, 24], window_size=window_size, ratio=4., scale=None, use_bias=True,
                           patch_normalize=True, dropout_rate=dropout_rate,
                           attention_dropout_rate=attention_dropout_rate, droppath_rate=droppath_rate,
                           normalize=normalize, activation=activation, conv=conv)
...
def swin_transformer_tiny_224_w7_22k(input_tensor=None, input_shape=None, window_size=7, classes=21841,
                                     include_top=True, dropout_rate=0., attention_dropout_rate=0., droppath_rate=0.1,
                                     normalize=normalize, activation=tfa.activations.gelu, weights="imagenet",
                                     conv=True):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    else:
...
def swin_transformer_tiny_224_w7_22kto1k(input_tensor=None, input_shape=None, window_size=7, classes=1000,
                                         include_top=True, dropout_rate=0., attention_dropout_rate=0.,
                                         droppath_rate=0.1, normalize=normalize, activation=tfa.activations.gelu,
                                         weights="imagenet", conv=True):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_small_224_w7_1k(input_tensor=None, input_shape=None, window_size=7, classes=1000, include_top=True,
                                     dropout_rate=0., attention_dropout_rate=0., droppath_rate=0.3, normalize=normalize,
                                     activation=tfa.activations.gelu, weights="imagenet", conv=True):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    else:
    ...
    out = swin_transformer(img_input, classes, include_top, patch_size=4, n_feature=96, n_blocks=[2, 2, 18, 2],
                           n_heads=[3, 6, 12, 24], window_size=window_size, ratio=4., scale=None, use_bias=True,
                           patch_normalize=True, dropout_rate=dropout_rate,
                           attention_dropout_rate=attention_dropout_rate, droppath_rate=droppath_rate,
                           normalize=normalize, activation=activation, conv=conv)
...
def swin_transformer_small_224_w7_22k(input_tensor=None, input_shape=None, window_size=7, classes=21841,
                                      include_top=True, dropout_rate=0., attention_dropout_rate=0., droppath_rate=0.2,
                                      normalize=normalize, activation=tfa.activations.gelu, weights="imagenet",
                                      conv=True):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
...
def swin_transformer_small_224_w7_22kto1k(input_tensor=None, input_shape=None, window_size=7, classes=1000,
                                          include_top=True, dropout_rate=0., attention_dropout_rate=0.,
                                          droppath_rate=0.2, normalize=normalize, activation=tfa.activations.gelu,
                                          weights="imagenet", conv=True):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    else:
...
def swin_transformer_base_224_w7_1k(input_tensor=None, input_shape=None, window_size=7, classes=1000, include_top=True,
                                    dropout_rate=0., attention_dropout_rate=0., droppath_rate=0.5, normalize=normalize,
                                    activation=tfa.activations.gelu, weights="imagenet", conv=True):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    else:
    ...
    out = swin_transformer(img_input, classes, include_top, patch_size=4, n_feature=128, n_blocks=[2, 2, 18, 2],
                           n_heads=[4, 8, 16, 32], window_size=window_size, ratio=4., scale=None, use_bias=True,
                           patch_normalize=True, dropout_rate=dropout_rate,
                           attention_dropout_rate=attention_dropout_rate, droppath_rate=droppath_rate,
                           normalize=normalize, activation=activation, conv=conv)
    model = tf.keras.Model(img_input, out)
...
def swin_transformer_base_224_w7_22k(input_tensor=None, input_shape=None, window_size=7, classes=21841,
                                     include_top=True, dropout_rate=0., attention_dropout_rate=0., droppath_rate=0.2,
                                     normalize=normalize, activation=tfa.activations.gelu, weights="imagenet",
                                     conv=True):
    if input_tensor is None:
...
def swin_transformer_base_224_w7_22kto1k(input_tensor=None, input_shape=None, window_size=7, classes=1000,
                                         include_top=True, dropout_rate=0., attention_dropout_rate=0.,
                                         droppath_rate=0.2, normalize=normalize, activation=tfa.activations.gelu,
                                         weights="imagenet", conv=True):
    if input_tensor is None:
...
def swin_transformer_base_384_w12_1k(input_tensor=None, input_shape=None, window_size=12, classes=1000,
                                     include_top=True, dropout_rate=0., attention_dropout_rate=0., droppath_rate=0.5,
                                     normalize=normalize, activation=tfa.activations.gelu, weights="imagenet",
                                     conv=True):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
...
def swin_transformer_base_384_w12_22k(input_tensor=None, input_shape=None, window_size=12, classes=21841,
                                      include_top=True, dropout_rate=0., attention_dropout_rate=0., droppath_rate=0.2,
                                      normalize=normalize, activation=tfa.activations.gelu, weights="imagenet",
                                      conv=True):
    if input_tensor is None:
    ...
    out = swin_transformer(img_input, classes, include_top, patch_size=4, n_feature=128, n_blocks=[2, 2, 18, 2],
                           ...
                           patch_normalize=True, dropout_rate=dropout_rate,
                           attention_dropout_rate=attention_dropout_rate, droppath_rate=droppath_rate,
                           normalize=normalize, activation=activation, conv=conv)(out)
    model = tf.keras.Model(img_input, out)
...
def swin_transformer_base_384_w12_22kto1k(input_tensor=None, input_shape=None, window_size=12, classes=1000,
                                          include_top=True, dropout_rate=0., attention_dropout_rate=0.,
                                          droppath_rate=0.2, normalize=normalize, activation=tfa.activations.gelu,
                                          weights="imagenet", conv=True):
    if input_tensor is None:
...
def swin_transformer_large_224_w7_22k(input_tensor=None, input_shape=None, window_size=7, classes=21841,
                                      include_top=True, dropout_rate=0., attention_dropout_rate=0., droppath_rate=0.2,
                                      normalize=normalize, activation=tfa.activations.gelu, weights="imagenet",
                                      conv=True):
    if input_tensor is None:
    ...
    out = swin_transformer(img_input, classes, include_top, patch_size=4, n_feature=192, n_blocks=[2, 2, 18, 2],
                           n_heads=[6, 12, 24, 48], window_size=window_size, ratio=4., scale=None, use_bias=True,
                           patch_normalize=True, dropout_rate=dropout_rate,
                           attention_dropout_rate=attention_dropout_rate, droppath_rate=droppath_rate,
                           normalize=normalize, activation=activation, conv=conv)
    model = tf.keras.Model(img_input, out)
...
def swin_transformer_large_224_w7_22kto1k(input_tensor=None, input_shape=None, window_size=7, classes=1000,
                                          include_top=True, dropout_rate=0., attention_dropout_rate=0.,
                                          droppath_rate=0.2, normalize=normalize, activation=tfa.activations.gelu,
                                          weights="imagenet", conv=True):
    if input_tensor is None:
...
def swin_transformer_large_384_w12_22k(input_tensor=None, input_shape=None, window_size=12, classes=21841,
                                       include_top=True, dropout_rate=0., attention_dropout_rate=0., droppath_rate=0.2,
                                       normalize=normalize, activation=tfa.activations.gelu, weights="imagenet",
                                       conv=True):
    if input_tensor is None:
...
def swin_transformer_large_384_w12_22kto1k(input_tensor=None, input_shape=None, window_size=12, classes=1000,
                                           include_top=True, dropout_rate=0., attention_dropout_rate=0.,
                                           droppath_rate=0.2, normalize=normalize, activation=tfa.activations.gelu,
                                           weights="imagenet", conv=True):
    if input_tensor is None:
...
def swin_transformer_v2_tiny_256_w8_1k(input_tensor=None, input_shape=None, window_size=8, classes=1000,
                                       include_top=True, dropout_rate=0., attention_dropout_rate=0., droppath_rate=0.2,
                                       normalize=normalize, activation=tfa.activations.gelu,
                                       pre_window_size=[8, 8, 8, 8], weights="imagenet", conv=True):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
    ...
    out = swin_transformer_v2(img_input, classes, include_top, patch_size=4, n_feature=96, n_blocks=[2, 2, 6, 2],
                              n_heads=[3, 6, 12, 24], window_size=window_size, ratio=4., use_bias=True,
                              patch_normalize=True, dropout_rate=dropout_rate,
                              attention_dropout_rate=attention_dropout_rate, droppath_rate=droppath_rate,
                              normalize=normalize, activation=activation, pre_window_size=pre_window_size, conv=conv)
    model = tf.keras.Model(img_input, out)
...
def swin_transformer_v2_tiny_256_w16_1k(input_tensor=None, input_shape=None, window_size=16, classes=1000,
                                        include_top=True, dropout_rate=0., attention_dropout_rate=0., droppath_rate=0.2,
                                        normalize=normalize, activation=tfa.activations.gelu,
                                        pre_window_size=[16, 16, 16, 8], weights="imagenet", conv=True):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_v2_small_256_w8_1k(input_tensor=None, input_shape=None, window_size=8, classes=1000,
                                        include_top=True, dropout_rate=0., attention_dropout_rate=0., droppath_rate=0.3,
                                        normalize=normalize, activation=tfa.activations.gelu,
                                        pre_window_size=[8, 8, 8, 8], weights="imagenet", conv=True):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    ...
    out = swin_transformer_v2(img_input, classes, include_top, patch_size=4, n_feature=96, n_blocks=[2, 2, 18, 2],
                              n_heads=[3, 6, 12, 24], window_size=window_size, ratio=4., use_bias=True,
                              patch_normalize=True, dropout_rate=dropout_rate,
                              attention_dropout_rate=attention_dropout_rate, droppath_rate=droppath_rate,
                              normalize=normalize, activation=activation, pre_window_size=pre_window_size, conv=conv)
...
def swin_transformer_v2_small_256_w16_1k(input_tensor=None, input_shape=None, window_size=16, classes=1000,
                                         include_top=True, dropout_rate=0., attention_dropout_rate=0.,
                                         droppath_rate=0.3, normalize=normalize, activation=tfa.activations.gelu,
                                         pre_window_size=[16, 16, 16, 8], weights="imagenet", conv=True):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    else:
...
def swin_transformer_v2_base_256_w8_1k(input_tensor=None, input_shape=None, window_size=8, classes=1000,
                                       include_top=True, dropout_rate=0., attention_dropout_rate=0., droppath_rate=0.5,
                                       normalize=normalize, activation=tfa.activations.gelu,
                                       pre_window_size=[8, 8, 8, 8], weights="imagenet", conv=True):
    if input_tensor is None:
    ...
    out = swin_transformer_v2(img_input, classes, include_top, patch_size=4, n_feature=128, n_blocks=[2, 2, 18, 2],
                              n_heads=[4, 8, 16, 32], window_size=window_size, ratio=4., use_bias=True,
                              patch_normalize=True, dropout_rate=dropout_rate,
                              attention_dropout_rate=attention_dropout_rate, droppath_rate=droppath_rate,
                              normalize=normalize, activation=activation, pre_window_size=pre_window_size, conv=conv)
    model = tf.keras.Model(img_input, out)
...
def swin_transformer_v2_base_256_w16_1k(input_tensor=None, input_shape=None, window_size=16, classes=1000,
                                        include_top=True, dropout_rate=0., attention_dropout_rate=0., droppath_rate=0.5,
                                        normalize=normalize, activation=tfa.activations.gelu,
                                        pre_window_size=[16, 16, 16, 8], weights="imagenet", conv=True):
    if input_tensor is None:
...
def swin_transformer_v2_base_192_w12_22k(input_tensor=None, input_shape=None, window_size=12, classes=21841,
                                         include_top=True, dropout_rate=0., attention_dropout_rate=0.,
                                         droppath_rate=0.2, normalize=normalize, activation=tfa.activations.gelu,
                                         pre_window_size=[12, 12, 12, 6], weights="imagenet", conv=True):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_v2_base_256_w16_22kto1k(input_tensor=None, input_shape=None, window_size=16, classes=1000,
                                             include_top=True, dropout_rate=0., attention_dropout_rate=0.,
                                             droppath_rate=0.2, normalize=normalize, activation=tfa.activations.gelu,
                                             pre_window_size=[12, 12, 12, 6], weights="imagenet", conv=True):
    if input_tensor is None:
...
def swin_transformer_v2_base_384_w24_22kto1k(input_tensor=None, input_shape=None, window_size=24, classes=1000,
                                             include_top=True, dropout_rate=0., attention_dropout_rate=0.,
                                             droppath_rate=0.2, normalize=normalize, activation=tfa.activations.gelu,
                                             pre_window_size=[12, 12, 12, 6], weights="imagenet", conv=True):
    if input_tensor is None:
...
def swin_transformer_v2_large_192_w12_22k(input_tensor=None, input_shape=None, window_size=12, classes=21841,
                                          include_top=True, dropout_rate=0., attention_dropout_rate=0.,
                                          droppath_rate=0.2, normalize=normalize, activation=tfa.activations.gelu,
                                          pre_window_size=[12, 12, 12, 6], weights="imagenet", conv=True):
    if input_tensor is None:
    ...
    out = swin_transformer_v2(img_input, classes, include_top, patch_size=4, n_feature=192, n_blocks=[2, 2, 18, 2],
                              n_heads=[6, 12, 24, 48], window_size=window_size, ratio=4., use_bias=True,
                              patch_normalize=True, dropout_rate=dropout_rate,
                              attention_dropout_rate=attention_dropout_rate, droppath_rate=droppath_rate,
                              normalize=normalize, activation=activation, pre_window_size=pre_window_size, conv=conv)
    model = tf.keras.Model(img_input, out)
...
def swin_transformer_v2_large_256_w16_22kto1k(input_tensor=None, input_shape=None, window_size=16, classes=1000,
                                              include_top=True, dropout_rate=0., attention_dropout_rate=0.,
                                              droppath_rate=0.2, normalize=normalize, activation=tfa.activations.gelu,
                                              pre_window_size=[12, 12, 12, 6], weights="imagenet", conv=True):
    if input_tensor is None:
...
def swin_transformer_v2_large_384_w24_22kto1k(input_tensor=None, input_shape=None, window_size=24, classes=1000,
                                              include_top=True, dropout_rate=0., attention_dropout_rate=0.,
                                              droppath_rate=0.2, normalize=normalize, activation=tfa.activations.gelu,
                                              pre_window_size=[12, 12, 12, 6], weights="imagenet", conv=True):
    if input_tensor is None:
